...
BonusStacking枚举是VCMI奖励系统中奖励叠加规则的定义，用于控制相同类型的奖励如何叠加。

## 类定义

```cpp
enum class DLL_LINKAGE BonusStacking
{
    NO_LIMIT,      // 无限制叠加
    INFINITE,      // 无限叠加（与NO_LIMIT类似）
    NORMAL,        // 标准叠加（通常为数值相加）
    WEAKEST,       // 取最弱（取最小值）
    STRONGEST,     // 取最强（取最大值）
    MULTIPLICATIVE // 乘法叠加
...
BonusStacking是VCMI奖励系统中用于定义奖励叠加规则的枚举。在复杂的游戏中，可能会有多个相同类型的奖励同时作用于一个对象，这时就需要确定这些奖励如何相互影响。有些奖励应该相加（如多个攻击力加成），有些应该取最大值（如多个护甲穿透效果），还有些应该相乘（如多个伤害加成）。这个枚举定义了这些不同的叠加策略。

## 枚举值

- `NO_LIMIT`: 无限制叠加，所有奖励都会累加，适用于通常应该累加的奖励类型
- `INFINITE`: 无限叠加，与NO_LIMIT类似，但可能在某些特殊情况下有不同的处理逻辑
- `NORMAL`: 标准叠加，通常是数值相加，是最常用的叠加方式
- `WEAKEST`: 取最弱，只保留最小值，适用于只取最优效果的情况
- `STRONGEST`: 取最强，只保留最大值，适用于同类效果只取最优的情况
- `MULTIPLICATIVE`: 乘法叠加，适用于需要相乘而不是相加的奖励类型

## 设计说明

BonusStacking枚举在VCMI奖励系统中起着关键作用，它确保了多个同类奖励能够以预期的方式结合，避免了奖励滥用或不当叠加导致的游戏失衡。通过合理选择叠加规则，游戏可以实现更丰富的策略深度和平衡性。
